...
public class MarketsListAdaptor extends RecyclerView.Adapter<MarketsListAdaptor.MyViewHolder> {
    ...
    class MyViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener {
        ...
        public void onClick(View view) {
            onItemClickListener.onMarketItemClick(marketsList.get(getLayoutPosition()));
        }
    }

    public interface OnItemClickListener {
        void onMarketItemClick(MarketObject item);
    }

    public MarketsListAdaptor(Activity activity, ArrayList<MarketObject> marketsList, OnItemClickListener onItemClickListener) {
    ...
    public void onBindViewHolder(@NonNull @NotNull MyViewHolder holder, int position) {
        ...
        MarketObject money = marketsList.get(position);

        holder.lblCurrencyName.setText(money.getStock());
        holder.lblCurrencySymbol.setText(money.getStock());
        holder.lblPrice.setText(utils.reduceDecimal(money.getPrice(), Integer.parseInt(money.getMoney_prec())));
        holder.lblPriceUnit.setText(money.getMoney());
...
//        }
//        holder.lblPriceChange.setText(utils.reduceDecimal(money.getPriceChange(),2));


        setChartViewProperties(holder.chartLine); // set chart view properties

        LineDataSet lineDataSet = new LineDataSet(money.getChartData(), "lineChart");
        ...
        if(!money.getStock().equals("null")) {
            FetchAndLoadImage fetchAndLoadImage = new FetchAndLoadImage(activity.getApplicationContext());
            fetchAndLoadImage.setImage(money.getStock(), holder.imgLogo, "ic_" + money.getStock());
        }
    }
